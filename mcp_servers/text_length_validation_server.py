#!/usr/bin/env python3
"""
Text Length Validation MCP Server
...
class TextLengthValidationMCPServer:
    """Text Length Validation MCP Server for TTS timing validation"""
    ...
    async def validate_text_timing(
        ...
        if estimated_duration <= max_seconds:
            status = "Ready"  # Changed from "Approved" to "Ready" for Airtable compatibility
        else:
        ...
        return {
            ...
            "message": f"{'✅' if status == 'Ready' else '⏳'} {scene_type.capitalize()} text: {duration_info['word_count']} words, ~{estimated_duration}s (limit: {max_seconds}s)"
        }
...
async def test_text_length_validation_server():
    ...
    server = TextLengthValidationMCPServer()
