            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self.base_url, params=params, timeout=30) as response:
                    ...
                    if len(final_products) < count:
                        print(f"❌ VALIDATION FAILED: Only found {len(final_products)} products with valid prices (needed {count})")
                        print(f"   This title does not have enough qualifying products!")
                        # Return empty list to signal failure
                        return []
                    
                    print(f"✅ VALIDATION PASSED: Found {len(final_products)} qualifying products")
