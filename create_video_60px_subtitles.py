#!/usr/bin/env python3
"""
Create video with 60PX FONT SIZE subtitles
- Style: "boxed-word" with individual word boxes
- Font: Montserat, size 60px (fixed pixel size)
...
async def create_video_60px_subtitles():
    """Create video with 60px font size subtitles"""
    ...
    # Load updated Test schema with 60px font size
    with open('/home/claude-workflow/Test_json2video_schema.json', 'r') as f:
        movie_json = json.load(f)
    
    print("🎬 Creating video with 60PX FONT SIZE subtitles")
    print("📏 Font size updated:")
    print("  ❌ OLD: font-size: '10vw' (responsive)")
    print("  ✅ NEW: font-size: 60 (fixed 60 pixels)")
    print("\n📦 Subtitle styling:")
    print("  🎨 Style: 'boxed-word' (individual word boxes)")
    print("  🔤 Font: Montserat, size 60px (medium-large)")
        ...
        if response.status_code == 200:
            ...
            if status_response.status_code == 200:
                ...
                if error:
                    ...
                    print(f"✅ No errors - 60px font size accepted!")
                ...
                # Show visual representation with 60px sizing
                print("\n🎨 Expected Subtitle Appearance (60px sizing):")
                print("  ┌──────────┐ ┌──────────┐ ┌──────────┐")
                print("  │ WELCOME  │ │    TO    │ │   OUR    │")
                print("  └──────────┘ └──────────┘ └──────────┘")
                print("    (green)     (yellow)     (green)")
                print("  📏 Font size: 60px (fixed, medium-large)")
                print("  📦 Each word in its own box")
                print("  🟢 Non-current words: Green background")
                print("  🟡 Current word: Yellow background")
                print("  ⚫ All words: Thick black outline (width 8)")
                print("  🎯 Perfect size for readability without being too large")
...
if __name__ == "__main__":
    print("🎬 JSON2Video - 60PX FIXED FONT SIZE")
    print("=" * 40)
    result = asyncio.run(create_video_60px_subtitles())
    
    if result['success']:
        print("\n🎉 SUCCESS! Video with 60px font size created!")
        print(f"🔗 View: {result['project_url']}")
        print("\n📏 60px Font Benefits:")
        print("  ✅ Fixed size (consistent across all videos)")
        print("  ✅ Medium-large readability")
        print("  ✅ Professional boxed-word styling")
        print("  ✅ Green/Yellow color scheme with black outline")
        print("  ✅ Perfect for mobile and desktop viewing")
    else:
